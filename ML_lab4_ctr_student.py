...
aSparse = SparseVector(aDense.shape[0],[1,3.],[3,4])

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(bDense.shape[0],[3],[1.])
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    parsePoints=parsePoint(point)
    
    item=point.split(',')
    label=item[0]
    features=oneHotEncoding(parsePoints,OHEDict,numOHEFeats)
    
    #map(lambda x:oneHotEncoding(x,sampleOHEDictManual,len(sampleOHEDictManual)))
...
# (OHETrainData.take(1)[0]).features[1]
...
def parseHashPoint(point, numBuckets):
    ...
    items=point.split(',')
    label=items[0]
    features=hashFunction(numBuckets,parsePoints,printMapping=False)
